...
from .bot_level_mapping import LEVEL_MAPPING, parse_state
...
class AllianceIDChannel(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.pending_state_answers = set()
    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if pending_key in self.pending_state_answers:
                return
            ...
            await self.process_fid(
                message, fid, alliance_id, given_state, ask_for_state=True
            )
    ...
    async def _ask_for_state(self, message, fid):
        key = (message.guild.id, message.channel.id, message.author.id)
        self.pending_state_answers.add(key)
        try:
            await message.reply(
                f"{theme.globeIcon} What state do you belong to? "
                f"Reply with the state number only, for example `1755`.",
                delete_after=60,
            )
            ...
            reply = await self.bot.wait_for(
                ...
                check=lambda answer: (
                    ...
                    and answer.author.id == message.author.id
                    and answer.channel.id == message.channel.id
                ),
            )
            state = parse_state(reply.content)
            if state is None:
                await reply.reply(
                    f"{theme.deniedIcon} That is not a valid state number. "
                    f"Post your game ID again to retry.",
                    delete_after=self.get_guild_settings(message.guild.id)["delete_after"],
                )
            return state
        except asyncio.TimeoutError:
            await message.reply(
                f"{theme.warnIcon} State entry timed out. Post your game ID again to retry.",
                delete_after=self.get_guild_settings(message.guild.id)["delete_after"],
            )
            return None
        finally:
            self.pending_state_answers.discard(key)

    async def process_fid(
        self, message, fid, alliance_id, given_state=None, ask_for_state=False
    ):
        settings = self.get_guild_settings(message.guild.id)
        delete_after = settings['delete_after']
        ...
        try:
            ...
            if given_state is None and ask_for_state:
                given_state = await self._ask_for_state(message, fid)
                if given_state is None:
                    return

            # Historical messages are scanned without an interactive wait. Mixed-state
            # entries therefore still need both values in the original message.
            if given_state is None and is_multistate(alliance_id):
                await message.add_reaction(theme.warnIcon)
                await message.reply(
                    f"{theme.infoIcon} Post your ID **and** state together, "
                    f"for example `{fid} 245`.",
                    delete_after=delete_after,
                )
                return
            ...
            if given_state is not None:
                kid, verified = parse_state(given_state), False
                if kid is None:
                    await message.add_reaction(theme.deniedIcon)
                    await message.reply(
                        f"{theme.deniedIcon} `{given_state}` is not a valid state number.",
                        delete_after=delete_after,
                    )
                    return
            else:
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"After a player posts their ID, the bot asks for their state. "
                    f"They can also post both together, e.g. `12345678 245`.\n"
                    f"{theme.upperDivider}\n"
                    f"{state_line}\n"
                    f"{theme.lowerDivider}"
                ),
